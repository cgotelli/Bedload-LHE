...
distMinIsolated = 18; %30; % Separation to define isolated particles. Necessary for getting a mean velocity.
...
%     imshow(images(:,:,300));
    ...
    tic
    final_particles = Particles_Filtered(particle_info, height, width, distMinIsolated,areamin,areamax,lim_width,lim_height); % Requieres particles, image's size and distance threshold.
...
%     plot_centroids(images, final_particles, FirstFrame, single) 
    ...
    tic
    [velocity,mvel] = mean_vel(final_particles, distMinVel, distMaxVel,1/60, difs_th, x_dev, savepath, filenames(j).name); %difs_th_M, difs_th_m % Matrix with timestep and correspondent velocity
    fprintf('We passed the mean velocity computation\n') % Prints file name
    toc
    plot_velocity(velocity,images,250);
%     plot_velocity(velocity,images,500);
%     plot_velocity(velocity,images,25);
%     plot_velocity(velocity,images,35);
%     plot_velocity(velocity,images,55);
